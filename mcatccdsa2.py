...
# TCC DSA - APLICAÇÃO DE ACM
# MBA em Data Science e Analytics USP ESALQ

# MARIANA DA SILVA ROSA DE OLIVEIRA
# -*- coding: utf-8 -*-
...
dados_mca = pd.read_excel("baseunicawvs2.xlsx")
# Fonte: adaptado de https://www.worldvaluessurvey.org/WVSDocumentationWV7.jsp
...
plt.figure(figsize=(10, 8))  # Tamanho maior para melhor visualização

# Criar o scatterplot com ajustes estéticos
scatter = sns.scatterplot(
    data=chart_df_mca, 
    x='obs_x', 
    y='obs_y', 
    hue='variavel', 
    s=60,  # Tamanho maior dos pontos
    palette='viridis',  # Paleta de cores mais acessível
    alpha=0.8,  # Transparência para pontos sobrepostos
    edgecolor='w',  # Borda branca nos pontos
    linewidth=0.5
)

# Linhas de eixo
plt.axhline(y=0, color='gray', ls='--', linewidth=0.8, alpha=0.7)
plt.axvline(x=0, color='gray', ls='--', linewidth=0.8, alpha=0.7)

# Rótulos dos pontos com ajustes
def label_point(x, y, val, ax):
    a = pd.concat({'x': x, 'y': y, 'val': val}, axis=1)
    for i, point in a.iterrows():
        ax.text(
            ...
            point['y'] - 0.02, 
            point['val'], 
            fontsize=8,  # Tamanho maior
            alpha=0.7,   # Transparência
            bbox=dict(facecolor='white', alpha=0.6, edgecolor='none', pad=1)  # Fundo branco
        )

label_point(
    x=chart_df_mca['obs_x'],
    y=chart_df_mca['obs_y'],
    val=chart_df_mca['categoria_id'],
    ax=plt.gca()
)
...
# Ajustes estéticos
sns.despine(top=True, right=True, left=False, bottom=False)
plt.tick_params(size=2, labelsize=9)

# Legenda melhor posicionada
plt.legend(
    bbox_to_anchor=(1.05, 1),  # Posição superior direita
    loc='upper left',
    frameon=True,
    framealpha=0.8,
    title='Variáveis',
...
# Títulos e rótulos
plt.title("Mapa Perceptual - Análise de Correspondência Múltipla (MCA)", 
          fontsize=14, pad=20)
plt.xlabel(f"Eixo 1: {tabela_autovalores.iloc[0,1]}% da inércia", 
           fontsize=10, labelpad=10)
plt.ylabel(f"Eixo 2: {tabela_autovalores.iloc[1,1]}% da inércia", 
           fontsize=10, labelpad=10)
